...
import { aiContexts } from '@/dev-utils/data/AiContextRetrieval';
...
export const VariableModal: FC<Props> = ({
  ...
  const handleSubmit = () => {
    if (updatedVariables.some((variable) => variable.value === ''
      && !aiContexts.find(aiContext => aiContext.key === variable.key))) {
    ...
    >
      ...
      >
        ...
        {updatedVariables.map((variable, index) => {
          const aiContext = aiContexts.find(aiContext => aiContext.key === variable.key);
          const initialValue = aiContext ? aiContext.context : "";
              ...
              </div>
              {
                aiContext ? (
                  <textarea
                    ...
                    className="mt-1 w-full rounded-lg border border-neutral-400 px-4 py-2 text-neutral-900 shadow focus:outline-none dark:border-neutral-700 dark:border-opacity-60 dark:bg-[#40414F] dark:opacity-50 dark:text-neutral-100"
                    style={{ resize: 'none' }}
